...
PAPERS_CSV     = Path(__file__).resolve().parents[1] / "data/processed/papers_v2.csv"
...
def s2_get(endpoint: str, params: dict | None = None) -> dict:
    ...
    for attempt in range(MAX_RETRY):
        ...
        if 400 <= r.status_code < 500:
            # e.g. 400 "offset + limit must be < 10000" for mega-authors (mis-disambiguated ids)
            log.warning("  HTTP %d for %s params=%s -> treating as end of data: %s",
                        r.status_code, endpoint, params, r.text[:120])
            return {"data": [], "_client_error": r.status_code}
        r.raise_for_status()
        return r.json()
...
def phase2b_per_author(paper_authors: dict, sample_paper_ids: set | None = None) -> dict:
    ...
    for i, aid in enumerate(remaining):
        ...
        while True:
            ...
            if offset + AUTHOR_PAGE_LIMIT > 10000:
                # S2 API hard cap: offset + limit must be < 10000. Authors with >~10k papers are
                # mis-disambiguated S2 ids; keep the (truncated) history and flag it.
                log.warning("  author %s has >%d papers; history truncated at API cap", aid, offset)
                papers_all.append({"paperId": "__TRUNCATED__", "year": None,
                                   "publicationDate": None, "arxivId": None})
                break
